...
class SensorConnector:
	...
	def _get_brightness(self, sensors_status):
		...
		if (sensors_status & 0x04):
			...
			return (self.bus.read_byte_data(DEVICE_ADDR, LIGHT_REG_H) << 8 |
					self.bus.read_byte_data(DEVICE_ADDR, LIGHT_REG_L))
	...
	def _get_pressure_sensor(self):
		...
		pressure = (self.bus.read_byte_data(DEVICE_ADDR, BMP280_PRESSURE_REG_L)|
	...
	def get_all_sensors(self):
		...
		sensors_dict['Date'] = time.strftime('%Y-%m-%d %H:%M:%S')
		...
		sensors_dict['TempOnChip'], sensors_dict['Humidity'] = self._get_on_chip_hum_sensor()

		sensors_dict['TempBaromter'], sensors_dict['Pressure'] = self._get_pressure_sensor()
